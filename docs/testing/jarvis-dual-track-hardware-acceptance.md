...
| Source lifecycle IPC, start isolation, and microphone recovery ordering | `PASS` | Strict IPC/preload tests plus renderer lifecycle tests verify one interruption, indefinite physical-microphone retry, durable restoration before replacement PCM, stale-callback suppression, retry after pipeline-attach failure, stop-invalidated pre-bind/initial mic/system attachment attempts, and post-worklet liveness checks that prevent dead candidates from being published |
| System-source interruption recording | `PASS` | Renderer loopback and current main-managed WASAPI failures each report one durable interruption. Automatic system producer re-enumeration/rebuild and restored-before-flush ordering are intentionally pending Task 8.1 |
...
| Full Jarvis, typecheck, lint, i18n, and renderer build gates | `PASS` | Fresh Task 8 verification ran `npm run test:jarvis`, `npm run typecheck`, `npm run lint`, `npm run i18n:check`, and `npm run build:renderer`; each exited 0 |
...
| 4 | Change the active Windows output device. If Windows process-loopback PCM remains continuous, verify there is no false PC gap. If the helper actually interrupts, verify one durable PC gap, uninterrupted microphone capture, and same-session PC recovery after installing the Task 8.1 system-rebuild implementation. | `NOT RUN` | `NOT RUN` | Requires a second playback device and a coordinated Windows routing change. The interrupted-helper recovery branch is blocked on Task 8.1; continuous process-loopback behavior may be valid and must not be forced into a false gap. |
...
durable interruption recording, sidecar reconciliation, and startup recovery. Automatic system-source
re-enumeration/rebuild remains a Task 8.1 implementation item. No physical Shure MV7, Windows
...
The Task 8 dual-track foundation automated gates may pass independently, but the phase is not
hardware-approved until Task 8.1 system-source rebuilding is implemented where interruption occurs
and every manual row above records an observed result and `PASS`, including the full two-hour soak.
...
- Task 8.1 must add automatic system-source re-enumeration/rebuild, restored-before-buffer-flush ordering,
  and stop-time retry cancellation before the interrupted-helper branch of step 4 can pass.
