...
const MAX_LOGICAL_CHUNK_BOUNDARY_OVERLAP_MS = 5_000;
...
function completeAudio(session, track, entries) {
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    if (
      ...
      chunk.channels <= 0 ||
      (previous &&
        previous.ended_at - chunk.started_at >
          (track.logical_track_id
            ? MAX_LOGICAL_CHUNK_BOUNDARY_OVERLAP_MS
            : MAX_CHUNK_BOUNDARY_OVERLAP_MS))
    ) {
      return false;
    }
    previous = chunk;
