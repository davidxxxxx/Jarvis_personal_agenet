...
test("deduplicates concurrent analysis and persists pseudonymous text-only results", async () => {
  ...
  const repository = {
    getSessionDetail: () => ({
      ...
      summary: null,
      segments: [
        { id: "seg-1", started_at: 2, ended_at: 3, person_id: "p-real", speaker_label: "张三", text: "讨论交付", is_stable: 1 },
      ],
    }),
  ...
  const client = {
    analyze: (input) => {
      ...
      return new Promise((done) => {
        resolve = () => done({ result: { summary: "交付", topics: [], memories: [], todos: [], decisions: [], suggestions: [] }, usage: { inputTokens: 1, outputTokens: 1 }, model: "MiniMax-M2.7" });
      });
    },
  };
  const scheduler = new AnalysisScheduler({ repository, client, createId: () => "run-1", now: () => 10 });
